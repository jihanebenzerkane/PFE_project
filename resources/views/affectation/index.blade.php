...
@push('styles')
    ...
    <div id="planningModal">
        <div class="modal-box">
            ...
            <p>Entrez uniquement la date de début. Le programme calculera automatiquement le nombre de jours nécessaires.
            </p>
    ...
    <div class="action-topbar">
        ...
        <div class="action-topbar-btns">
            ...
            <form action="{{ route('affectation.run') }}" method="POST" style="display:inline;">
                ...
                @else
                    <button type="submit" class="btn btn-primary"
                        style="background:#4F46E5; font-size:1rem; padding:10px 22px;">
            ...
            <button type="button" class="btn btn-success"
                onclick="document.getElementById('planningModal').style.display='flex'">📅 Générer le Planning</button>
                        ...
                        <tbody>
                            @forelse($projets as $i => $projet)
                                <tr>
                                    ...
                                    <td>
                                        @if ($projet->etudiant)
                                            <div class="student-name">{{ $projet->etudiant->nom }}
                                                {{ $projet->etudiant->prenom }}</div>
                                            <div class="student-sub">{{ $projet->etudiant->cne }}</div>
                                        @endif
                                        @if ($projet->etudiant2)
                                            <div style="border-top: 1px dashed #E2E8F0; margin: 6px 0; padding-top: 6px;">
                                            </div>
                                            <div class="student-name">{{ $projet->etudiant2->nom }}
                                                {{ $projet->etudiant2->prenom }}</div>
                                    ...
                                    <td>
                                        @php
                                            ...
                                            if (
                                                str_contains($f, 'TDIA') ||
                                                str_contains($f, 'TRANSFORM') ||
                                                str_contains($f, 'ARTIFIC')
                                            ) {
                                                $filiereCode = 'tdia';
                                            } elseif (
                                                str_contains($f, 'INGÉNIERIE') ||
                                                str_contains($f, 'INGENIERIE') ||
                                                str_contains($f, 'DONNÉES') ||
                                                str_contains($f, 'DONNEES') ||
                                                str_contains($f, 'ID')
                                            ) {
                                                $filiereCode = 'id';
                                            } elseif (
                                                str_contains($f, 'GÉNIE') ||
                                                str_contains($f, 'GENIE') ||
                                                str_contains($f, 'GI')
                                            ) {
                                                $filiereCode = 'gi';
                                            }
                            ...
                            @empty
                                <tr>
                                    <td colspan="5" style="text-align:center; color:#94A3B8; padding: 40px;">
                                        Aucun projet importé. <a href="{{ route('import.form') }}"
                                            style="color:#3B82F6;">Importer un fichier Excel</a>
                                    </td>
                                </tr>
                            @endforelse
                        </tbody>
...
@push('scripts')
    ...
    <script>
        function addDate() {
            ...
            row.innerHTML =
                '<input type="date" name="dates[]" required><button type="button" class="btn-remove" onclick="removeDate(this)">✕</button>';
